...
public class KhoaDAO {
    ...
    public static boolean addKhoa(String ma, String ten) {
        ...
        String checkSql = "SELECT COUNT(*) FROM Khoa WHERE TenKhoa = ?";
        try (Connection conn = ConnectDatabase.getConnection();
                PreparedStatement pCheck = conn.prepareStatement(checkSql)) {
            pCheck.setString(1, ten);
    ...
    public static boolean isKhoaExist(String maKhoa) {
        try (Connection conn = ConnectDatabase.getConnection();
                PreparedStatement p = conn.prepareStatement("SELECT COUNT(*) FROM Khoa WHERE MaKhoa = ?")) {
    ...
    public static boolean addMonHoc(String ma, String ten, int tc, String maKhoa) {
        if (ma == null || ma.trim().isEmpty() || ten == null || ten.trim().isEmpty() || tc < 0) {
            return false;
        }
        String checkSql = "SELECT COUNT(*) FROM MonHoc WHERE TenMon = ?";
        try (Connection conn = ConnectDatabase.getConnection();
                PreparedStatement pCheck = conn.prepareStatement(checkSql)) {
            pCheck.setString(1, ten);
            ResultSet rs = pCheck.executeQuery();
    ...
    public static boolean isMonHocExist(String maMon) {
        try (Connection conn = ConnectDatabase.getConnection();
                PreparedStatement p = conn.prepareStatement("SELECT COUNT(*) FROM MonHoc WHERE MaMon = ?")) {
            p.setString(1, maMon);
            ResultSet rs = p.executeQuery();
            if (rs.next()) {
                return rs.getInt(1) > 0;
